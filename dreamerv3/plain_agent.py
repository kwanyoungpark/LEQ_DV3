...
class Agent(nj.Module):
  ...
  def __init__(self, obs_space, act_space, config):
    ...
    print(self.config.critic)
    critic_config = {**self.config.critic}
    critic_config['inputs'] = ['deter', 'stoch']
    # Critic
    self.critic = nets.MLP((), name='critic', **critic_config)
    self.slowcritic = nets.MLP(
        (), name='slowcritic', **critic_config, dtype='float32')
  ...
  def loss(self, data, carry, update=True):
    ...
    val = critic.mean()[:,:,0] * vscale + voffset
    ...
    losses['critic'] = sg(weight)[:, :-1, None] * -(
        critic.log_prob(sg(ret_padded)[:,:,None]) +
    ...
    if self.config.replay_critic_loss:
      ...
      losses['replay_critic'] = sg(f32(~data['is_last']))[:, :-1, None] * -(
        replay_critic.log_prob(sg(ret_padded)[:,:,None]) +
