...
class RSSM(nj.Module):
  ...
  def initial(self, bsize):
    if self.markov:
        print(self.stoch, self.classes)
        carry = dict(
            deter=jnp.zeros([bsize, self.stoch * self.classes + 6], f32),
  ...
  def observe(self, carry, action, embed, reset, bdims=2):
    ...
    if self.markov:
        _stoch = stoch.reshape((stoch.shape[0], -1))
        deter = feat = jnp.concatenate([_stoch, action], axis=-1)
        x = embed
    else:
        if self.actless: action = jnp.zeros_like(action)
  ...
  def get_penalty(self, carry, action):
    assert self.ensemble > 1
    if isinstance(action, dict):
      action = jaxutils.concat_dict(action)
    B, T = carry['deter'].shape[:2]
    fn = lambda x: x.reshape((B*T, *x.shape[2:]))
    carry = jax.tree_util.tree_map(fn, carry)
    action = jax.tree_util.tree_map(fn, action)
    carry, action = cast((carry, action))
    if self.markov:
        _stoch = carry['stoch'].reshape((carry['stoch'].shape[0], -1))
        deter = feat = jnp.concatenate([_stoch, action], axis=-1)
    else:
        if self.actless: action = jnp.zeros_like(action)
        deter, feat = self._gru(carry['deter'], carry['stoch'], action)
        if self.actless: feat = jnp.concatenate([feat, action], axis=-1)
    logit = self._prior(feat, output_all=True)
    z = self._dist(logit).mean()
    mz = jnp.mean(z, axis=0, keepdims=True)
    penalty = jnp.linalg.norm(z-mz, axis=(-2,-1)).mean(axis=0)
    penalty = penalty.reshape((B, T, *penalty.shape[1:]))
    return penalty
  ...
  def loss(self, outs, free=1.0):
    metrics = {}
    prior = self._prior(outs.get('feat', outs['deter']), output_all = True)
    post = outs['logit'][None]
  ...
  def _prior(self, feat, output_all=False):
    kw = dict(**self.kw, norm=self.norm, act=self.act)
    x = feat
    if self.ensemble > 1:
      logits = []
      for j in range(self.ensemble): 
        for i in range(self.imglayers):
          x = self.get(f'img{i}_{j}', Linear, self.hidden, **kw)(x)
        logit = self._logit(f'imglogit_{j}', x)
        logits.append(logit)
      logits = jnp.stack(logits, axis=0)
      if output_all is False:
          index = jax.random.choice(nj.seed(), self.ensemble, ())
          logit = logits[index]
          #index = jax.random.choice(nj.seed(), self.ensemble, (1, *logits.shape[1:-1], 1))
          #logit = jnp.take_along_axis(logits, index, axis=0)[0] 
      else:
          logit = logits
    else:
      for i in range(self.imglayers):
        x = self.get(f'img{i}', Linear, self.hidden, **kw)(x)
      logit = self._logit('imglogit', x)
    return logit
...
class SimpleEncoder(nj.Module):
  ...
  def __call__(self, data, bdims=2):
    ...
    if self.imgkeys:
      ...
      x = x.reshape((-1, *x.shape[bdims:]))
      #jax.debug.print("EMBED_INPUT {x}", x=jax.tree_util.tree_map(jnp.mean, x))
      for i, depth in enumerate(self.depths):
        stride = 1 if self.outer and i == 0 else 2
        x = self.get(f'conv{i}', Conv2D, depth, self.kernel, stride, **kw)(x)
        #jax.debug.print(f"EMBED_FEAT {i}: "+"{x}", x=jax.tree_util.tree_map(jnp.mean, x))
...
class MLP(nj.Module):
  ...
  def __init__(self, shape, dist='mse', inputs=['tensor'], **kw):
    ...
    distonly = ('outscale', 'minstd', 'maxstd', 'unimix', 'bins', 'ensemble')
...
class TruncNormalDist(tfd.TruncatedNormal):
    ...
    def sample(self, *args, **kwargs):
        ...
        if self._clip:
            ...
            event = event - sg(event) + sg(clipped)
        if self._mult:
            event *= self._mult
        return event
    ...
    def log_prob(self, x, *args, **kwargs):
        if self._clip:
            clipped = jnp.clip(x, self.low + self._clip, self.high - self._clip)
            x = x - sg(x) + sg(clipped)
        #jax.debug.print('ACT {x} {y}', x=x.min(), y=x.max())
        res = super().log_prob(x, *args, **kwargs)
        #jax.debug.print('LOGP {x} {y}', x=res.min(), y=res.max())
        return res

class _TanhNormalDist(tfd.TransformedDistribution):
    def __init__(self, distribution):
        super().__init__(distribution, tfb.Tanh())

    def sample(self, **kwargs):
        x_t = self.distribution.sample(**kwargs)
        y_t = jnp.tanh(x_t)
        action = y_t 
...
class TanhNormalDist():
    def __init__(self, distribution):
        ...
        self.name = self.dist.name
        self.batch_shape = self.dist.batch_shape[:-1]

    def __getattr__(self, name):
        return getattr(self.dist, name)

    def sample(self, **kwargs):
        x_t = self.dist.sample(**kwargs)
        y_t = jnp.tanh(x_t)
        action = y_t 

        log_prob = self.dist.log_prob(x_t)
        log_prob -= jnp.log(1 - (y_t ** 2) + 1e-6)
        #log_prob = log_prob.sum(axis=-1)
        return action, log_prob

    def mode(self):
        return jnp.tanh(self.dist.mode())

def actor_log_prob(dist, actor_dists, ys, eps=1e-6):
  logpi = {}
  for k in dist:
    if actor_dists[k] == 'tanh_normal':
      y = jnp.clip(ys[k], -1+eps, 1-eps)
      x = jnp.arctanh(y)
      log_prob = dist[k].log_prob(x) - jnp.log(1 - (y ** 2) + eps).sum(axis=-1)
      logpi[k] = log_prob 
    else:
      logpi[k] = dist[k].log_prob(ys[k])
  return cast(logpi)

@jax.custom_jvp
def tanh_penalty(x):
  y = jnp.tanh(x)
  penalty = jnp.log(1 - (y ** 2) + 1e-6)
  return penalty

@tanh_penalty.defjvp
def grad_tanh_penalty(xs, grads):
  x, = xs
  grad, = grads
  value = tanh_penalty(x)
  grad = -2 * jnp.tanh(x) * grad
  return value, grad

def actor_sample(dist, actor_dists, eps=1e-6):
  act, logpi, logpi_sg = {}, {}, {}
  for k in dist:
    if actor_dists[k] == 'tanh_normal':
      x_t = dist[k].sample(seed=nj.seed())
      y_t = jnp.tanh(x_t)
      act[k] = y_t

      log_prob = dist[k].log_prob(x_t)
      log_prob -= tanh_penalty(x_t).sum(axis=-1)
      logpi[k] = log_prob 

      log_prob_sg = dist[k].log_prob(sg(x_t))
      log_prob_sg -= tanh_penalty(sg(x_t)).sum(axis=-1)
      logpi_sg[k] = log_prob_sg 
    else:
      act[k] = cast(dist[k].sample(seed=nj.seed()))
      logpi[k] = dist[k].log_prob(act[k])
      logpi_sg[k] = dist[k].log_prob(sg(act[k]))
  return cast(act), cast(logpi), cast(logpi_sg)
...
class Dist(nj.Module):
  ...
  def __call__(self, inputs):
    ...
    if self.ensemble == 0:
        ...
        assert tuple(dist.batch_shape)[:-1] == tuple(inputs.shape[:-1]), (dist.batch_shape, dist.event_shape, inputs.shape)
    return dist
  ...
  def inner(self, inputs):
    ...
    if 'twohot' in self.dist or self.dist == 'softmax':
      ...
      if self.ensemble == 0:
        ...
        shape = (*self.shape, self.ensemble, self.bins + padding)
    elif self.ensemble > 0:
        shape = (*self.shape, self.ensemble)
    ...
    if self.dist == 'tanh_normal':
      lo, hi = self.minstd, self.maxstd
      std = (hi - lo) * jax.nn.sigmoid(std + 2.0) + lo
      dist = tfd.Normal(out, std) # TANH is not here
      #dist = TanhNormalDist(dist)
      dist = tfd.Independent(dist, len(self.shape))
      dist.minent = 0.0
      dist.maxent = 1.0
      #dist.minent = np.prod(self.shape) * tfd.Normal(0.0, lo).entropy()
    ...
    if self.dist == 'trunc_normal':
      lo, hi = self.minstd, self.maxstd
      std = (hi - lo) * jax.nn.sigmoid(std + 2.0) + lo
      dist = TruncNormalDist(jnp.tanh(out), std, -1, 1)
      dist = tfd.Independent(dist, len(self.shape))
      dist.minent = np.prod(self.shape) * (
          TruncNormalDist(1.0, lo, -1, 1).entropy())
      dist.maxent = np.prod(self.shape) * (
          TruncNormalDist(0.0, hi, -1, 1).entropy())
      return dist
...
class Linear(nj.Module):
  ...
  def __init__(self, units):
    ...
    if self.outscale > 0:
        ...
        self._oinit = lambda: jnp.ones((), dtype=jaxutils.PARAM_DTYPE)
        self._winit = Initializer(
            self.winit, self.outscale, self.fan, self.dtype
        )
    self._binit = Initializer('zeros', 1.0, self.fan, self.dtype)
  ...
  def _layer(self, x):
    ...
    if self.outscale > 0.0: 
        kernelW = np.array(1.0, dtype=x.dtype)
        kernel = self.get('kernel', self._winit, shape, fan_shape).astype(x.dtype)
    else:
        kernelW = sg(self.get('kernelW', self._oinit).astype(x.dtype))
        kernel = self.get('kernel', self._winit, shape, fan_shape).astype(x.dtype)
    x = x @ (kernel * kernelW)
    #if self.outscale < 0.005:
    #    jax.debug.print(self.path + ' kernelW {x}', x=kernelW)
    #    jax.debug.print(self.path + ' kernel {x}', x=kernel.std())
