...
class JAXAgent(embodied.Agent):
  ...
  def __init__(self, agent_cls, obs_space, act_space, config):
    ...
    self._stack = jax.jit(lambda xs: jax.tree_util.tree_map(
        jnp.stack, xs, is_leaf=lambda x: isinstance(x, list)))
    self._split = jax.jit(lambda xs: jax.tree_util.tree_map(
  ...
  @embodied.timer.section('jaxagent_eval_policy')
  def eval_policy(self, obs, obss, firsts, T):
    obs = self._filter_data(obs)
    if obss is None:
      obss = {k: np.zeros((T, *obs[k].shape)) for k in obs}
      self.masks = np.zeros((T, self.length, 1))
      obss = {k: np.concatenate([obss[k][:T-1], obs[k][None]], axis=0) for k in obs}
      firsts = np.concatenate([firsts[:T-1], jnp.ones_like(firsts[-1:])], axis=0)
    else:
      obss = np.concatenate([obss[:T-1], obs[None]], axis=0)
      firsts = np.concatenate([firsts[:T-1], jnp.zeros_like(firsts[-1:])], axis=0)

    with embodied.timer.section('upload_inputs'):
      with self.policy_lock:
        obss = jax.device_put(obss, self.policy_sharded)
        seed = self._next_seeds(self.policy_sharded)

    with embodied.timer.section('jit_policy'):
      with self.policy_lock:
        acts, outs, carry = self._policy(
            self.policy_params, obss, firsts, seed, mode
        )
    ...
    with embodied.timer.section('swap_params'):
      with self.policy_lock:
        if self.pending_sync:
          old = self.policy_params
          self.policy_params = self.pending_sync
          jax.tree_util.tree_map(lambda x: x.delete(), old)
          self.pending_sync = None
  ...
  def train(self, data, carry):
    ...
    if self.should_sync(self.updates) and not self.pending_sync:
      ...
      jax.tree_util.tree_map(lambda x: x.delete(), allo)
  ...
  def load(self, state):
    with self.train_lock:
      with self.policy_lock:
        ...
        jax.tree_util.tree_map(lambda x: x.delete(), self.params)
        jax.tree_util.tree_map(lambda x: x.delete(), self.policy_params)
        self.params = jax.device_put(state, self.train_mirrored)
        self.policy_params = jax.device_put(
            {k: self.params[k].copy() for k in self.policy_keys},
            self.policy_mirrored)

  @embodied.timer.section('jaxagent_load_model')
  def load_model(self, state):
    with self.train_lock:
      with self.policy_lock:
        from pprint import pprint
        pprint(jax.tree_util.tree_map(jnp.shape, self.params))
        wm_params = [param for param in self.params if 'enc' in param or 'dyn' in param or 'dec' in param or 'rew' in param or 'con' in param]
        params = jax.device_get(self.params)
        for key in wm_params:
            print("LOADED", key)
            params[key] = jax.device_put(state[key])
  ...
  def _transform(self):
    ...
    def report(params, data, carry, seed):
      ...
      _, (mets, carry) = pure(params, data, carry, seed=seed, create=True)
      mets = {k: v[None] for k, v in mets.items()}
      return mets, carry
  ...
  def _take_mets(self, mets):
    mets = jax.tree_util.tree_map(lambda x: x.__array__(), mets)
  ...
  def _take_outs(self, outs):
    outs = jax.tree_util.tree_map(lambda x: x.__array__(), outs)
