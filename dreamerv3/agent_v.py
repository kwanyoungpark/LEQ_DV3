...
class Agent(nj.Module):
  ...
  def loss(self, data, carry, update=True):
    ...
    # Return
    critic = self.value({**outs})
    slowcritic = self.slowvalue({**outs})
    ...
    #### LEQ ####
    adv_weight = jnp.where(adv > 0, expectile, 1 - expectile)
    adv = adv_weight * adv
    logpi = sum(logpis_sg.values())
    ents = {k: -logpis[k][:, :-1] if self.actor_dists[k] == 'tanh_normal' else v.entropy()[:, :-1] for k, v in actor.items()}
    ...
    if self.config.replay_critic_loss:
      ...
      replay_acts = jaxutils.onehot_dict({'action': data['action']}, self.act_space)
