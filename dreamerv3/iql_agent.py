...
class Agent(nj.Module):
  ...
  def train(self, data, carry):
    ...
    if self.config.run.model_checkpoint:
        mets, outs = self.opt(
            [self.critic_modules, self.actor_modules],
            [self.critic_loss, self.actor_loss],
            data, carry, has_aux=True,
        )
    else:
        mets, outs = self.opt(
            [self.wm_modules],#self.critic_modules, self.actor_modules], 
            [self.wm_loss],#self.critic_loss, self.actor_loss, ],
            data, carry, has_aux=True,
        )
  ...
  def loss(self, data, carry, update=True):
    critic_loss, outs = self.critic_loss(data, carry, update)
    actor_loss, _ = self.actor_loss(data, carry, update)
    if self.config.run.model_checkpoint:
        loss = critic_loss + actor_loss
    else:
        wm_loss, _ = self.wm_loss(data, carry, update)
        loss = critic_loss + actor_loss + wm_loss
  ...
  def report(self, data, carry):
    ...
    print("CONTEXT0", nj.context().keys())
    img_outs = self.dyn.imagine(img_start, img_acts)[1]
    print("CONTEXT1", nj.context().keys())
    #print(awefawf)
    rec = dict(
