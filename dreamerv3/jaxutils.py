...
class Optimizer(nj.Module):
  ...
  def __call__(self, modules, lossfns, *args, has_aux=False, **kwargs):
    #def wrapped(lossfn, *args, **kwargs):
    #  outs = lossfn(*args, **kwargs)
    def update_recursive(dict1, dict2):
        res = {}
        for key in dict1: res[key] = dict1[key]
        for key in dict2:
            if key in res and isinstance(res[key], dict):
                print(res[key], dict2[key])
                res[key] = update_recursive(res[key], dict2[key])
            else: res[key] = dict2[key]
        return res

    def wrapped0(*args, **kwargs):
      outs = lossfns[0](*args, **kwargs)
    ...
    wrapped_lossfns = [wrapped0, wrapped1, wrapped2][:len(lossfns)]
    #wrapped_lossfns = [(lambda *args, **kwargs: wrapped(lossfn, *args, **kwargs)) for lossfn in lossfns]
    metrics = {}
    loss, params, grads, aux = 0., {}, {}, {}

    all_modules = []
    for (wrapped_lossfn, module) in zip(wrapped_lossfns, modules):
        #print(jax.tree_util.tree_map(jnp.mean, wrapped_lossfn(*args, **kwargs)))
        _loss, _params, _grads, _aux = nj.grad(
            wrapped_lossfn, module, has_aux=True
        )(*args, **kwargs)
        loss += _loss
        params.update(_params)
        for k in _grads:
            if k in grads: grads[k] += _grads[k]
            else: grads[k] = _grads[k]
        aux = update_recursive(aux, _aux)
        for _module in module: 
            if _module not in all_modules:
                all_modules.append(_module)
    #jax.debug.print("Loss {x}", x=jax.tree_util.tree_map(jnp.mean, loss)) 
    #jax.debug.print("Params {x}", x=jax.tree_util.tree_map(jnp.mean, params))
    #jax.debug.print("Grads {x}", x=jax.tree_util.tree_map(jnp.mean, grads))
    #jax.debug.print("Aux {x}", x=jax.tree_util.tree_map(jnp.mean, aux))
    if self.scaling:
    ...
    if self.scaling:
      invscale = 1.0 / (self.grad_scale.read() + 1e-12)
    ...
    param_norm = optax.global_norm([x.find() for x in all_modules])
    isfin = jnp.isfinite
    if self.scaling:
