...
def main(argv=None):
  ...
  from . import agent_dynamics as agt
...
def make_logger(config):
  ...
  logger = embodied.Logger(step, [
      ...
      embodied.logger.WandBOutput(config.task, project="dreamerv3_LEQ", config=dict(config)),
  ], multiplier)
  return logger
...
def make_replay(config, directory=None, is_eval=False, rate_limit=False):
  ...
  if suite == 'dmc':
    fnames = glob.glob(os.path.join(os.getcwd(), f'data/v-d4rl/{task}/{diff}/64px/*.npz'))
  if suite == 'd4rl':
    task = task.replace('_', '-')
    ...
    if 'antmaze' in task:
        ...
        # diff: diverse
        if diff is None:
            fnames = glob.glob(os.path.join(os.getcwd(), f'./data/d4rl/{task}-v0/*.npz'))
        else:
...
def make_env(config, index, **overrides):
  ...
  if suite == 'd4rl': task = task.replace('_', '-')
