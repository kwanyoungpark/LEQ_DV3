...
class Agent(nj.Module):
  ...
  def policy(self, obs, carry, mode='train'):
    ...
    if not self.config.plan:
        ...
        actor = self.actor(
          treemap(
            lambda x: jnp.repeat(x[:, None], M, axis=1), out
          ), bdims=2
        )
        act = sample(actor)
        startlat = treemap(
            lambda x: jnp.repeat(x[:, None], M * P, axis=1).reshape((B*M*P, *x.shape[1:])),
            cast(lat),
        )
        startact = treemap(
            lambda x: jnp.repeat(x[:, :, None], P, axis=2).reshape((B*M*P, *x.shape[2:])),
            cast(act),
        )
        startout = treemap(
            lambda x: jnp.repeat(x[:, None], M * P, axis=1).reshape((B*M*P, *x.shape[1:])),
        ...
        ret = rets[-1]
        ret = jnp.reshape((B, M, P))
        ret_low = jnp.min(ret, axis=2)
        ret_high = jnp.max(ret, axis=2)
        for _ in range(30):
            ret_mid = (ret_low + ret_high)[:, :, None] / 2
            w = jnp.where(ret > ret_mid, expectile, 1 - expectile)
            grad = jnp.sum(w * (ret - ret_mid), axis=1)
            ret_low = jnp.where(grad < 0, ret_low, ret_mid)
            ret_high = jnp.where(grad > 0, ret_high, ret_mid)
        jax.debug.print('EXPECTILE {x} {y}', x=ret_low[0,0], y=jnp.mean(ret,axis=-1)[0,0])
        idxs = jnp.argmax(ret_low, axis=1)
        act = treemap(
            lambda x: jnp.take_along_axis(x, idxs, axis=1),
            act,
        )
  ...
  def loss(self, data, carry, update=True):
    ...
    #jax.debug.print('logpi {x} {y}', x=logpi.min(), y=logpi.max())
    #jax.debug.print('entropy {x} {y}', x=logpi.min(), y=logpi.max())
    #jax.debug.print('adv {x} {y}', x=adv.min(), y=adv.max())
    ...
    #jax.debug.print('critic {x} {y}', x=losses['critic'].min(), y=losses['critic'].max())
    # Value
    losses['value'] = sg(weight) * -(
        value.log_prob(sg(Atarval))
    )

    if self.config.replay_critic_loss == 'onestep':
    ...
    elif self.config.replay_critic_loss == 'lamb':
      replay_acts = {'action': data['action']}
      replay_acts = jaxutils.onehot_dict(replay_acts, self.act_space)
      replay_critic = self.critic({**replay_outs, **replay_acts})
      replay_slowcritic = self.slowcritic({**replay_outs, **replay_acts})
      ...
      voffset, vscale = self.valnorm(replay_ret, update)
      ret_normed = (replay_ret - voffset) / vscale
      ret_padded = jnp.concatenate([ret_normed, 0 * ret_normed[:, -1:]], 1)[:, :, None]
      losses['replay_critic'] = sg(f32(~data['is_last']))[:, :-1, None] * -(
          replay_critic.log_prob(sg(ret_padded)) +
          self.config.slowreg * replay_critic.log_prob(sg(replay_slowcritic.mean()))
)[:, :-1]
