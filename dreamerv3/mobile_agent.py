...
class Agent(nj.Module):
  ...
  def policy(self, obs, carry, mode='train'):
    ...
    #jax.debug.print("OBS {x}", x=jax.tree_util.tree_map(jnp.mean, obs))
    obs = self.preprocess(obs)
    #jax.debug.print("OBS_P {x}", x=jax.tree_util.tree_map(jnp.mean, obs))
    embed = self.enc(obs, bdims=1)
    #jax.debug.print("EMBED {x}", x=jax.tree_util.tree_map(jnp.mean, embed))
    prevact = jaxutils.onehot_dict(prevact, self.act_space)
    lat, out = self.dyn.observe(
        prevlat, prevact, embed, obs['is_first'], bdims=1)
    #jax.debug.print("LAT {x}", x=jax.tree_util.tree_map(jnp.mean, lat))
    #jax.debug.print("OUT {x}", x=jax.tree_util.tree_map(jnp.mean, out))
  ...
  def get_penalty(self, carry, action):
    ...
    deter, feat = self.dyn._gru(carry['deter'], carry['stoch'], action)
    logit = self.dyn._prior(feat, output_all=True)
    stoch = cast(self.dyn._dist(logit).sample(seed=nj.seed()))
    deter = jnp.repeat(deter[None], stoch.shape[0], axis=0)
    carry = dict(deter=deter, stoch=stoch)
    outs = dict(deter=deter, stoch=stoch, logit=logit)

    print("outs", treemap(jnp.shape, outs)) 
    value = self.value(outs).mode()
    print("VALUE", value.shape)
    penalty = value.std(axis=0)
  ...
  def loss(self, data, carry, update=True):
    ...
    # Imagination rollout
    def imgstep(carry, _):
      # IMPORTANT CHANGE HERE (GRADIENT)
      lat, act = carry
      ...
      return (lat, act), (sg(out), sg(act), logpi, logpi_sg)
    ...
    adv = (critic.mean() - value.mean()) / rscale
    ...
    '''
    # Actor
    delta_a = {k: jnp.zeros_like(v) for k, v in acts.items()}
    def actor_fn(delta_a):
        _acts = {k: acts[k] + delta_a[k] for k in acts}
        v = self.critic({**sg(outs), **acts}).mean()
        return v.sum()

    grads = jax.grad(actor_fn)(delta_a)
    grad_a = {k: (grads[k] * acts[k]).sum(axis=-1) for k in grads}

    actor = self.actor(outs)
    ents = {k: v.entropy() for k, v in actor.items()}
    losses['actor'] = sg(weight) * -(
        sum(grad_a.values())
        + self.config.actent * sum(ents.values())
    )
    '''
    ...
    actor = self.actor(outs) 
    q_values = self.critic({**outs, **acts}).mean()
    q_values = q_values.mean(axis=-1)
    baseline = self.value({**outs}).mean()
    ...
    next_actions = {k: v[:, 1:] for (k, v) in acts.items()}
    next_critic = self.critic({**next_outs,**next_actions}).mean()

    ret = rew[:, :, None] + con[:, :, None] * discount * next_critic
    ret_padded = jnp.concatenate([ret, 0 * ret[:, -1:]], 1)
    losses['critic'] = sg(weight)[:, :, None] * -(
    ...
    if self.config.replay_critic_loss:
      ...
      replay_critic = self.critic({**replay_outs, **sg(replay_acts)})
      replay_slowcritic = self.slowcritic({**replay_outs, **sg(replay_acts)})

      replay_next_outs = {k: v[:, 1:] for (k, v) in replay_outs.items()}
      replay_next_actions = sample(self.actor(replay_next_outs))
      replay_next_v = self.critic({**replay_next_outs, **sg(replay_next_actions)}).mean()
      ...
      ret_padded = jnp.concatenate([replay_ret, 0 * replay_ret[:, -1:]], 1)
      losses['replay_critic'] = isvalid[:, :-1, None] * -(
