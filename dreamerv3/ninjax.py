...
def get_annotations(o):
    if isinstance(o, type):
        ann = o.__dict__.get('__annotations__', {})
    else:
        ann = getattr(o, '__annotations__', {})
    return ann
...
class ModuleMeta(type):
  ...
  def __new__(mcs, name, bases, clsdict):
    ...
    cls.__field_defaults = {
        k: getattr(cls, k) for k, v in get_annotations(cls).items()
        if hasattr(cls, k)}
    for key, value in get_annotations(cls).items():
  ...
  def __call__(cls, *args, name=None, **kwargs):
    ...
    if not re.match(r'^[A-Za-z0-9_]+$', name):
      raise ValueError(
          ...
          f'got: {name}')
    fields = {}
    for key, typ in get_annotations(cls).items():
      if key in kwargs:
