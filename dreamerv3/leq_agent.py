...
class Agent(nj.Module):
  ...
  def train(self, data, carry):
    ...
    mets, outs = self.opt(
        [self.wm_modules, self.actor_modules, self.critic_modules],
        [self.wm_loss, self.actor_loss, self.critic_loss],
  ...
  def loss(self, data, carry, update=True):
    actor_loss, actor_outs = self.actor_loss(data, carry, update)
    critic_loss, critic_outs = self.critic_loss(data, carry, update)
    wm_loss, wm_outs = self.wm_loss(data, carry, update)
    loss = actor_loss + critic_loss + wm_loss

    outs = wm_outs['outs']
    carry = wm_outs['carry']
    metrics = {**actor_outs['metrics'], **critic_outs['metrics'], **wm_outs['metrics']}
  ...
  def actor_loss(self, data, carry, update=True):
    ...
    stds = {k: v.scale for k, v in actor.items()}
    ...
    metrics.update({f'{k}_loss': v.mean() for k, v in losses.items()})
    metrics.update({f'{k}_loss_std': v.std() for k, v in losses.items()})
    metrics.update(jaxutils.tensorstats(adv_weight, 'adv_weight_actor'))
  ...
  def critic_loss(self, data, carry, update=True):
    metrics, losses = {}, {}
    prevlat, prevact = carry
    ...
    if self.config.replay_critic_loss:
      ...
      bc_loss = -sum([replay_actor[k].log_prob(replay_acts[k]) for k in replay_actor])
    ...
    metrics.update({f'{k}_loss': v.mean() for k, v in losses.items()})
    metrics.update({f'{k}_loss_std': v.std() for k, v in losses.items()})
    metrics.update(jaxutils.tensorstats(adv_weight, 'adv_weight_critic'))
    ...
    metrics.update(jaxutils.tensorstats(ret_normed, 'ret_normed'))
    if self.config.replay_critic_loss:
      metrics.update(jaxutils.tensorstats(replay_ret, 'replay_ret'))
    metrics['td_error'] = jnp.abs(ret - val[:, :-1]).mean()

    metrics['data_rew/min'] = jnp.abs(data['reward']).min()
    metrics['pred_rew/min'] = jnp.abs(rew).min()
