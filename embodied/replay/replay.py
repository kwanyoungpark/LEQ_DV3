...
class Replay:
  ...
  def _sample(self):
    ...
    while True:
      try:
        ...
        #print(chunkid, index, seq)
        #print(chunkid, index, {(k, seq[k].shape) for k in seq})
        #print(seq['reward'])
        #print('REW_seq', np.min(seq['reward']), np.max(seq['reward']), np.array(seq['reward']).shape)
        #print('TERM_seq', np.min(seq['is_terminal']), np.max(seq['is_terminal']), np.array(seq['is_terminal']).shape)
  ...
  def _insert(self, chunkid, index, loading=False):
    ...
    #print(stepids.shape)
    #print("stepids:", stepids, "index:", index, "capacity:", self.capacity, "real", len(self.items))
  ...
  def _assemble_batch(self, seqs, start, stop):
    ...
    for n, seq in enumerate(seqs):
      st, dt = 0, 0  # Source and destination time index.
          ...
          #    for _seq in seq[k]:
          #        print("-", _seq.shape)
          #print({(k, [_seq.shape for _seq in seq[k]]) for k in seq})
  ...
  def load(self, data=None, directory=None, amount=None):
    ...
    is_loading = data and 'limiter' in data
      ...
      with self.refs_lock:
        ...
        for chunk in reversed(chunks):
          ...
          for index in range(amount):
            self._insert(chunk.uuid, index, loading=is_loading)
  ...
  def _numitems(self, chunks):
    ...
    numitems = {}
    for uuid, succ in zip(uuids, succs):
      #print(lengths[uuid], self.length, future.get(succ, 0), uuid, succ)
