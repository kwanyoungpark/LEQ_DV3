...
def train(make_agent, make_replay, make_env, make_logger, args):
  replay = make_replay()
  for uuid in replay.chunks:
    print("REW_ALL", replay.chunks[uuid].data['reward'].min(), replay.chunks[uuid].data['reward'].max())
  agent = make_agent()
  logger = make_logger()
  ...
  def train_step():
      for uuid in replay.chunks:
        print("REW_ALL", uuid, replay.chunks[uuid].data['reward'].min(), replay.chunks[uuid].data['reward'].max())
      with embodied.timer.section('dataset_next'):
        batch = next(dataset_train)
      rew = jax.device_get(batch['reward'])
      for i in range(rew.shape[0]):
          #print(f"REWREW {i}", rew[i].min(), rew[i].max(), rew[i].shape)
          if rew[i].max() == np.inf:
              for k in batch:
                  print(k, batch[k][i].min(), batch[k][i].max())
