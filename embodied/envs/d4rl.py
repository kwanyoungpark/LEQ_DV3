...
class D4RL(embodied.Env):
  def __init__(self, env, obs_key='vector', act_key='action', **kwargs):
    if isinstance(env, str):
      self.env_name = env
      if 'antmaze' in env:
        if 'ultra' in env:
          self._env = gym.make(env + "-diverse-v0", **kwargs)
        else:
          self._env = gym.make(env + "-diverse-v2", **kwargs)
      else:
        self._env = gym.make(env + "-random-v2", **kwargs)
  ...
  def _obs(
    ...
    if 'antmaze' in self.env_name: 
      ...
      self.env.viewer.cam.distance = 6
